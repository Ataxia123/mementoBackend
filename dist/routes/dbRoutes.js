                ...
                if (res.statusCode && (res.statusCode >= 200 && res.statusCode < 400)) {
                    ...
                    res.on('end', function () {
                        parser.parseString(data, async function (err, result) {
                            ...
                            return await itemCollection.updateOne({ itemId: itemId }, { $setOnInsert: itemData }, { upsert: true });
...
dbRouter.post('/attest', async (req, res) => {
    ...
    async function payRespects(playerData) {
        ...
        await respects.updateOne({ id: playerData.uid }, { $setOnInsert: attestationData }, { upsert: true });
        await respectsTally.updateOne({ id: playerData.id }, { $push: { attestations: attestationData } }, { upsert: true });
    }
