...
import { Document, storageContextFromDefaults, VectorStoreIndex, SimpleMongoReader, MongoDBAtlasVectorSearch, serviceContextFromDefaults, ContextChatEngine, } from "llamaindex";
...
async function newDay(capname) {
    ...
    const response = await queryEngine.query(`Generate a status report from the prespective of the the following captain: ${capname}
`);
    // Output response
...
aiuRouter.post('/attest', async (req, res) => {
    ...
    async function payRespects(pilot, pilotAttestation, shipData, address, heroCodexId) {
        ...
        await quipux.updateOne({ id: oc.uid }, { $setOnInsert: pilotRecord }, { upsert: true });
    }
    try {
