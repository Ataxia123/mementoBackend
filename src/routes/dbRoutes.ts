...
dbRouter.post('/db', async (req: Request, res: Response) => {
    ...
    async function savePlayerItems(item: any, playerData: any) {
    ...
    async function savePlayerAndItems(playerData: any) {
        const playerItems = playerData.equipped_items || [];

        try {
            await savePlayer(playerData);
            await Promise.all(playerItems?.map((item: any) => savePlayerItems(item, playerData)));
        } catch (error) {
...
dbRouter.post('/attest', async (req, res) => {
    ...
    // function to save player
    async function payRespects(playerData: any) {
        // Save only if player id does not exist
        const attestationData = {
            id: playerData.uid,
            hero: playerData.id,
